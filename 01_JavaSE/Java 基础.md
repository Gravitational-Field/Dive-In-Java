...
1. 自动装箱发生在从一个数字构建一个包装类对象。
2. 自动拆箱发生在从一个包装类对象给一个变量赋值。
...
System.out.println(x == y);    // false 两个对象不是同一个对象
Integer z = Integer.valueOf(123);
Integer k = Integer.valueOf(123);
System.out.println(z == k);   // true   均访问的是IntegerCache.cache[123+128]的对象
Integer p = Integer.valueOf(128);
Integer q = Integer.valueOf(128);
System.out.println(p == q);  // false 128超出了缓冲池的存储范围，两个对象均是新new出来的对象
```
...
public static Integer valueOf(int i) {
    if (i >= IntegerCache.low && i <= IntegerCache.high)
        // 默认-128—127第一次会存，第二次访问时将会再次访问这个值。
...
编译器会在自动装箱过程调用 valueOf() 方法，因此多个值相同且值在缓存池范围内的 Integer 实例使用自动装箱来创建，那么就会引用相同的对象。 **所以避免这个坑的解决方案可以直接创建包装类型对象来避免。**
